...
% ------------------ TÍTULO ------------------
\title{\textbf{Aproximación manual a la fotometría de apertura}\\Trabajo sobre el cúmulo abierto Messier 41\\Segunda parte: Optimización de parámetros de proper motions}
...
para definir la pertenencia de una estrella a un cúmulo abierto. El presente se enfoca en la determinación de pertenencia mediante un método de optimización de parámetros gaussianos
a los movimientos propios de un grupo de estrellas estudiado por GAIA en su entrega de datos dr3. A raíz de esto, y siguiendo con el primer estudio que se hizo, 
se plantea una discusión sobre la pertinencia del método y la necesidad de profundizar el conocimiento del astrofísico en el dominio de los métodos computacionales y el machine learning, 
no como una herramienta útil pero como una necesidad para el avance de esta rama. 
\end{abstract}

\noindent\textbf{Palabras clave:} Messier 41, Optimización, Vector Point Diagram, Probabilidad de pertenencia. 
\vspace{0.5cm}
]
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\linewidth]{../CosasErrNorm/Distancia_en_sigmas.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1\linewidth]{../CosasErrNorm/VPD_Pmayor0_zoomcluster.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\linewidth]{../CosasErrNorm/Distribucion_espacial_RA_Dec.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1\linewidth]{../Lab2Otrocsv/CosasErrNorm/CMDPintadoErr.png}
...
ya que se esperarían unos 100 miembros en el cluster entre lo reportado en la literatura \cite{wiki} y el número, bastante rudimientario, encontrado en el estudio anterior de 111 centroides. 
Sin embargo, el CMD muestra muchos miembros en la zona de enanas blancas, antes de entrar a la secuencia principal, y pocos miembros en la misma, lo que es de esperarse en un estudio como este. 
...
Aunque no sea del todo necesario hablarlo, es cierto que es bueno mirar a las razones de por qué el autor trabajó por tanto tiempo con un set de datos
equivocado sin saber que no eran datos correctos. El ejemplo estrella es la comparación de (Fig~\ref{VPDcompleto cluster viejo}) y (Fig~\ref{VPDcompleto cluster nuevo}), 
donde la similitud en la región de los VPD pudo inducir un falso sentido de seguridad con respecto a los datos en el autor. Además, es cierto que el autor
nunca pensó haber descargado datos erróneos, por lo que podía sentirse seguro del contenido de su archivo. 

El leve retraso en la entrega de este documento se debe en parte a que esta confusión de datos generó la necesidad de comparar los archivos uno a uno. 
El archivo Laboratorio 2/Lab2Otrocsv/comparacióncsv.py contiene todas las pruebas que se realizaron para determinar la similitud de ambos documentos. 
Es también importante mencionar que, debido a la magnitud de estos informes, el autor tuvo que migrar su medio de trabajo a uno local para poder 
tratar la cantidad de imágenes que se ponen en este documento. Temas de documentación, citación y rutas en formato LaTeX causaron retrasos 
significantes en el trabajo del autor. 
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1\linewidth]{../CosasErrNorm/CMDPintadoErr.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1\linewidth]{../CosasErrNorm/VPDColoreado.png}
    \caption{VPD coloreado con el cluster del primer set de datos estudiado. }
    \label{VPDcompleto cluster viejo}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=1\linewidth]{../Lab2Otrocsv/CosasErrNorm/VPDColoreado.png}
    \caption{VPD coloreado con el cluster del segundo set de datos estudiado. }
    \label{VPDcompleto cluster nuevo}
\end{figure}
