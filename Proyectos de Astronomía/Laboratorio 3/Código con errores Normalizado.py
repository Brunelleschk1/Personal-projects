...
carpeta = "Proyectos de Astronomía/Laboratorio 3/"

# Carga de datos
df = pd.read_csv(carpeta + "M41_query_result_full.csv")
...
plt.tight_layout()
plt.savefig(carpeta + "VPDColoreado")
plt.close()
...
plt.savefig(carpeta + "CMDPintadoErr")
plt.close()
...
plt.savefig(carpeta + "Distancia_en_sigmas")
plt.close()
...
N_cluster, _ = np.histogram(r[P>0.7], bins=bins)
...
plt.scatter(ra[P>0.7], dec[P>0.7], s=10)
plt.gca().invert_xaxis()
plt.xlabel("RA")
plt.ylabel("DEC")
plt.title("Distribución espacial")
plt.savefig(carpeta + "Distribucion_espacial_RA_Dec")
plt.close()
...
def plot_vpd_prob_nonzero(pmra, pmdec, P, mu_xc, mu_yc, sigma_c):
    ...
    plt.tight_layout()
    plt.savefig(carpeta + "VPD_Pmayor0_zoomcluster")
    plt.close()
